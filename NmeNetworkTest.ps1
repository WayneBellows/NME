...
The hostname of the paired RTI app service (e.g., 'nmw-rti-app-ede5a.azurewebsites.net'). Required for testing bidirectional connectivity between NME and RTI.
...
# Add paired RTI app service if provided
if ($PairedAppServiceHostname) {
    $ApiEndpoints += [PSCustomObject]@{ URI = $PairedAppServiceHostname; Port = 443; Purpose = "Paired RTI App Service"; Exceptions = @(); DnsServer = $RemoteDns }
}
